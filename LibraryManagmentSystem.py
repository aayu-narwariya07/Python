...
def save_library(library):
    try:
        ...
        print(f"Eror saving library data: {e}")
...
# Task4:
# Function to view all books in the libaray
...
def borrow_book(library, title, borrowed_name):
    if title not in library:
        print("Book not found in the library.")
        return
    if library[title]['quantity'] ==0:
        print(f"The copies of '{title}' are currently borrowed.")
        return
    ...
    library[title]['borrowed_by'] = borrowed_name
    save_library(library)
    print(f"'{title}'has been borrowed by {borrowed_name}.")
...
# Task7:
# Main funcation to run the library managment system
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            author = input("Enter the book auther: ")
            try:
